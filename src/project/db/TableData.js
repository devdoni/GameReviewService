const TableData = {
   korPopular :[
        {name: 'PUBG: BATTLEGROUNDS', genre: '오픈월드/FPS', src: '/imgs/smailthums/battlegrounds.jpg', info: '넓은 오픈월드에서 친구들과 함께 팀을 만들어 전장에서 최후의 생존자가 되어보세요!', href: 'detail/1' },
        {name: 'Phasmophobia', genre: '공포/호러', src: '/imgs/smailthums/pahasmophobia.jpg', info: '4인용 협동 공포 게임입니다. 처분 가능한 모든 유령 사냥 장비를 사용하여 가능한 한 많은 증거를 수집해야 합니다.', href: 'detail/107'},
        {name: 'Sekiro™: Shadows Die Twice', genre: '액션', src: '/imgs/smailthums/sekiro.jpg', info: '복수를 향한 자신만의 길을 개척해보십시오. 복수하십시오. 명예를 회복하십시오. 무사답게 싸우십시오.', href: 'detail/28'},
        {name: 'Half-Life Alyx', genre: '공포/FPS', src: '/imgs/smailthums/alyx.jpg', info: 'Half-Life: Alyx는 Valve가 선보이는 Half-Life 시리즈의 새로운 VR 게임입니다.', href: 'detail/102'},
        {link: 'popular'}
   ],
   korFree :[
        {name: 'Apex 레전드™',genre: '배틀로얄/슈팅',src: '/imgs/smailthums/apex_legends.jpg',info: 'Titanfall 시리즈의 세계를 배경으로 한 빠른 속도의 배틀로얄 게임입니다. 각기 다른 능력을 가진 Legend들을 선택하여 팀을 이루어 싸우세요.', href: 'detail/101'},
        {name: 'Warframe',genre: '액션/RPG',src: '/imgs/smailthums/warframe.jpg',info: '우주를 배경으로 한 빠른 속도의 3인칭 슈팅 게임으로, 다양한 캐릭터와 무기를 사용하여 미션을 수행하고 적을 물리치세요.', href: 'detail/45'},
        {name: 'Destiny 2',genre: 'MMO/슈팅',src: '/imgs/smailthums/destiny2.jpg',info: '다양한 클래스와 무기를 선택하여 인류의 마지막 도시를 방어하는 게임입니다. 혼자서 또는 친구들과 함께 우주를 탐험하고 적을 물리치세요.', href: 'detail/18'},
        {name: 'Brawlhalla',genre: '플랫폼/격투',src: '/imgs/smailthums/brawlhalla.jpg',info: '최대 8명의 플레이어가 동시에 싸울 수 있는 플랫폼 격투 게임입니다. 다양한 모드와 캐릭터를 선택하여 친구들과 경쟁하세요.', href: 'detail/103'},
        {link: 'free'},
],
    korHorror :[
        {name: 'Escape the Backrooms', genre: '공포/호러', src: '/imgs/smailthums/backrooms.jpg', info: '이스케이프 더 백룸은 1~4인 협동 공포 탐험 게임입니다. 으스스한 밀실 레벨을 가로지르며 유령과 기타 위험을 피하고 탈출을 시도하세요', href: 'detail/103'},
        {name: 'Lethal Company', genre: '공포/호러', src: '/imgs/smailthums/letalcompony.jpg', info: '버려진 달에서 폐기물을 모아 회사에 판매하는 협동 공포 게임입니다.', href: 'detail/35' },
        {name: 'Dead by Daylight', genre: '공포/호러', src: '/imgs/smailthums/DeadbyDaylight.jpg', info: '한 명의 플레이어가 살인마의 역할을 담당하고, 나머지 네 명의 플레이어는 살인마로부터 도망치려는 생존자로 플레이하는 (4vs1) 공포게임 입니다.', href: 'detail/72'},
        {name: 'Project Zomboid', genre: '공포/생존', src: '/imgs/smailthums/projectzomboid.jpg', info: 'Project Zomboid는 좀비로 가득 찬 개방형 샌드박스 게임입니다. 한 가지 간단한 질문을 던집니다 – 당신은 어떻게 죽을 것인가?', href: 'detail/21'},
        {link: 'popular'}
],
    korSimulation : [
        {name: 'Euro Truck Simulator 2', genre: '운송/시뮬레이션', src: '/imgs/smailthums/EuroTruck.jpg', info: '유럽 전역을 여행하며 다양한 화물을 운송할 수 있는 트럭 시뮬레이션 게임입니다. 현실적인 운전 경험과 다양한 경로를 탐험해보세요.', href: 'detail/34'},    
        {name: 'Railway Empire 2', genre: '경영/시뮬레이션', src: '/imgs/smailthums/RailwayEmpire2.jpg',info: '19세기 미국의 철도 회사를 경영하며 철도망을 확장하고 회사를 성장시키는 시뮬레이션 게임입니다.', href: 'detail/104'},
        {name: 'Farming Simulator 22', genre: '농업/시뮬레이션', src: '/imgs/smailthums/FarmingSimulator22.jpg', info: '농작물 재배, 가축 사육 등 실제 농장 경영을 체험할 수 있는 시뮬레이션 게임입니다.', href: 'detail/105'},
        {name: 'Cities Skylines', genre: '도시 건설/시뮬레이션', src: '/imgs/smailthums/CitiesSkylines.jpg', info: '현대 도시를 설계하고 관리하며 다양한 도전 과제를 해결하는 도시 건설 시뮬레이션 게임입니다.', href: 'detail/106'},
        {link: 'popular'}
],


    engPopular :[
        {name: 'PUBG: BATTLEGROUNDS', genre: 'Open World/FPS', src: '/imgs/smailthums/battlegrounds.jpg', info: 'Create a team with friends in a vast open world and strive to be the last survivor on the battlefield!'},
        {name: 'Phasmophobia', genre: 'Horror', src: '/imgs/smailthums/pahasmophobia.jpg', info: 'A 4-player co-op horror game. Use all the ghost hunting equipment at your disposal to collect as much evidence as you can.'},
        {name: 'Sekiro™: Shadows Die Twice', genre: 'Action', src: '/imgs/smailthums/sekiro.jpg', info: 'Forge your own path to vengeance. Restore your honor. Fight with the way of the warrior.'},
        {name: 'Half-Life Alyx', genre: 'Horror/FPS', src: '/imgs/smailthums/alyx.jpg', info: 'Half-Life: Alyx is Valve’s VR return to the Half-Life series.'}
        
    ],

    engFree :[
        {name: 'Apex Legends', genre: 'Battle Royale/Shooter', src: '/imgs/smailthums/apex_legends.jpg', info: 'A fast-paced battle royale game set in the Titanfall universe. Choose from a variety of Legends, each with unique abilities, and form a team to fight.'},
        {name: 'Warframe', genre: 'Action/RPG', src: '/imgs/smailthums/warframe.jpg', info: 'A fast-paced third-person shooter set in space. Use various characters and weapons to complete missions and defeat enemies.'},
        {name: 'Destiny 2', genre: 'MMO/Shooter', src: '/imgs/smailthums/destiny2.jpg', info: 'Choose from various classes and weapons to defend the last city of humanity. Explore the universe and defeat enemies solo or with friends.'},
        {name: 'Brawlhalla', genre: 'Platform/Fighting', src: '/imgs/smailthums/brawlhalla.jpg', info: 'A platform fighting game where up to 8 players can battle simultaneously. Choose from various modes and characters to compete with friends.'}
    ],

    engHorror :[
        {name: 'Escape the Backrooms', genre: 'Horror', src: '/imgs/smailthums/alyx.jpg', info: 'Escape the Backrooms is a 1-4 player co-op horror exploration game. Traverse eerie levels avoiding ghosts and other dangers while trying to escape.'},
        {name: 'Lethal Company', genre: 'Horror', src: '/imgs/smailthums/letalcompony.jpg', info: 'A cooperative horror game where you collect scrap on an abandoned moon and sell it to a company.'},
        {name: 'Dead by Daylight', genre: 'Horror', src: '/imgs/smailthums/DeadbyDaylight.jpg', info: 'A (4vs1) horror game where one player takes on the role of the killer, and the other four play as survivors trying to escape the killer.'},
        {name: 'Project Zomboid', genre: 'Horror/Survival', src: '/imgs/smailthums/projectzomboid.jpg', info: 'Project Zomboid is an open-ended zombie-infested sandbox. It asks one simple question – how will you die?'}
    ],

    engSimulation :[
        {name: 'Euro Truck Simulator 2', genre: 'Transport/Simulation', src: '/imgs/smailthums/EuroTruck.jpg', info: 'Travel across Europe as a truck driver, delivering various cargoes. Experience realistic driving and explore diverse routes.'},
        {name: 'Railway Empire 2', genre: 'Management/Simulation', src: '/imgs/smailthums/RailwayEmpire2.jpg', info: 'Manage a railway company in 19th century America, expanding your rail network and growing your company.'},
        {name: 'Farming Simulator 22', genre: 'Agriculture/Simulation', src: '/imgs/smailthums/FarmingSimulator22.jpg', info: 'Experience real farming with crop cultivation, livestock breeding, and managing a farm.'},
        {name: 'Cities Skylines', genre: 'City Building/Simulation', src: '/imgs/smailthums/CitiesSkylines.jpg', info: 'Design and manage a modern city, solving various challenges in this city-building simulation game.'}
    ],

    chinaPopular :[
        {name: '绝地求生：大逃杀', genre: '开放世界/第一人称射击', src: '/imgs/smailthums/battlegrounds.jpg', info: '在广阔的开放世界中与朋友组队，争取成为战场上的最后幸存者！'},
        {name: 'Phasmophobia', genre: '恐怖', src: '/imgs/smailthums/pahasmophobia.jpg', info: '一款4人合作恐怖游戏。使用所有可用的猎鬼设备收集尽可能多的证据。'},
        {name: '只狼：影逝二度', genre: '动作', src: '/imgs/smailthums/sekiro.jpg', info: '开辟属于自己的复仇之路。恢复你的荣誉。以武士之道战斗。'},
        {name: '半衰期：爱莉克斯', genre: '恐怖/第一人称射击', src: '/imgs/smailthums/alyx.jpg', info: '半衰期：爱莉克斯是Valve推出的半衰期系列的新VR游戏。'}
    ],
    
    chinaFree :[
        {name: 'Apex英雄', genre: '大逃杀/射击', src: '/imgs/smailthums/apex_legends.jpg', info: '一款快速节奏的大逃杀游戏，设定在Titanfall宇宙中。选择具有独特能力的传奇角色，组成团队战斗。'},
        {name: 'Warframe', genre: '动作/角色扮演', src: '/imgs/smailthums/warframe.jpg', info: '一款快节奏的第三人称射击游戏，背景设定在太空。使用各种角色和武器完成任务并击败敌人。'},
        {name: '命运2', genre: '大型多人在线/射击', src: '/imgs/smailthums/destiny2.jpg', info: '选择各种职业和武器来保卫人类最后的城市。独自或与朋友一起探索宇宙并击败敌人。'},
        {name: 'Brawlhalla', genre: '平台/格斗', src: '/imgs/smailthums/brawlhalla.jpg', info: '一款平台格斗游戏，最多可有8名玩家同时战斗。选择各种模式和角色与朋友竞争。'}
    ],
    
    chinaHorror :[
        {name: '逃离后室', genre: '恐怖', src: '/imgs/smailthums/backrooms.jpg', info: '逃离后室是一款1-4人合作恐怖探险游戏。在阴森的密室中穿行，避开幽灵和其他危险，尝试逃脱。'},
        {name: '致命公司', genre: '恐怖', src: '/imgs/smailthums/letalcompony.jpg', info: '一款合作恐怖游戏，在废弃的月球上收集废料并卖给公司。'},
        {name: '黎明杀机', genre: '恐怖', src: '/imgs/smailthums/DeadbyDaylight.jpg', info: '一款（4对1）的恐怖游戏，一名玩家扮演杀手，其他四名玩家扮演幸存者，尝试逃离杀手。'},
        {name: '僵尸毁灭工程', genre: '恐怖/生存', src: '/imgs/smailthums/projectzomboid.jpg', info: '僵尸毁灭工程是一款开放式的僵尸沙盒游戏。它提出了一个简单的问题 - 你将如何死亡？'}
    ],
    
    chinaSimulation :[
        {name: '欧洲卡车模拟2', genre: '运输/模拟', src: '/imgs/smailthums/euro_truck_simulator_2.jpg', info: '在欧洲各地旅行，作为卡车司机运送各种货物。体验真实的驾驶并探索各种路线。'},
        {name: '模拟农场22', genre: '农业/模拟', src: '/imgs/smailthums/farming_simulator_22.jpg', info: '体验真实的农业经营，包括种植作物、饲养牲畜等。'},
        {name: '城市：天际线', genre: '城市建设/模拟', src: '/imgs/smailthums/cities_skylines.jpg', info: '设计并管理一个现代城市，解决各种挑战的城市建设模拟游戏。'},
        {name: '铁路帝国2', genre: '管理/模拟', src: '/imgs/smailthums/railway_empire_2.jpg', info: '在19世纪的美国管理一家铁路公司，扩展你的铁路网络并发展你的公司。'}
    ]

}
export default TableData;